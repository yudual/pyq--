...
- **后台管理**：动态、文章、评论、媒体、友链、音乐、黑名单、SMTP、高德地图、豆瓣、RSS 与站点设置；
...
## 3. 图片与媒体存储方案（支持自有图床 与 Cloudflare R2）

本博客系统提供**两种完全平行的图片使用模式**：

1. **自有图床 / 外部图片链接（零后端配置·开箱即用）**：
   - 如果您已有自己的图床（七牛云、又拍云、阿里云 OSS、腾讯云 COS、自建 Chevereto、兰空图床、GitHub/jsDelivr 等），**无需配置任何后端存储变量或 R2 凭据**！
   - 系统所有图片字段（博主头像、主页封面、背景轮播、文章/项目封面、正文插图、网站图标、OG卡片、友链等）均原生支持直接输入/粘贴以 `https://` 开头的图片 URL。
   - 前端已配置 Next.js 全域通配图片代理放行，任何合规外部图片外链均可自适应加载与压缩，绝不报错。
   - **注意点**：请确保您的自有图床未开启限制严格的防盗链（需在图床控制台将博客域名加入白名单，或允许空 Referer）。

2. **Cloudflare R2 对象存储（后台一键上传·媒体素材库联动）**：
   - 适合希望在后台直接点击“上传图片 / 视频”按钮将文件存入私有云、并在「媒体素材库（`/admin/media`）」中集中管理的用户。
   - Cloudflare R2 每月提供 10GB 免费存储额度，且**完全免流量费（零出网流量费）**。
   - 大文件与视频通过浏览器预签名直传 R2，不受 Vercel 函数体积限制。
   - 若选择此模式，请按照下文 3.1 ~ 3.3 完成配置。
...
6. 如启用了豆瓣影单，确认后端已配置 `CRON_SECRET`，并检查当前 Vercel 套餐是否支持 Cron Jobs。若不支持，可使用外部调度器定时调用该接口。
